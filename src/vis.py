...
def plot_test_history(test_history, test_history_by_onto, save=False):
    ...
    sns.lineplot(
        x="epoch", y="val_loss", hue="run", errorbar="sd", data=test_history, ax=ax1
    )
    ax1.set_ylabel("BCE loss")
    ax1.legend(loc="upper right")
    ax1.grid()
...
def report(ontos, y, Y, idx, thresh=0.5, save=False):
    ...
    if save:
        plt.savefig(save.replace("%", "curves") + ".png")
    ...
    if save:
        df.round(4).to_csv(save.replace("%", "stats") + ".csv")
...
def vis_pizza(
    ...
    for expr in manual_exprs:
        ...
        if isinstance(expr, tuple) or expr in {TOP, BOT}:
            ...
            if fact.check_sub(expr, C.VegetarianPizza):
                c = "lime"
            if fact.check_sub(expr, C.NonVegetarianPizza):
                c = "brown"

            if fact.check_sub(expr, C.VegetarianTopping):
                c = "lime"
            if fact.check_sub(expr, C.VegetableTopping):
                c = "green"
            if fact.check_sub(expr, C.MeatTopping):
                c = "brown"
            if fact.check_sub(expr, C.SeafoodTopping):
                c = "pink"
            if fact.check_sub(expr, C.Spiciness):
                c = "red"
    ...
    for x, y, text, color, marker, expr in zip(
        xs, ys, texts, colors, markers, manual_exprs
    ):
