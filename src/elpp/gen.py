...
def random_axiom(n_concepts: int, n_roles: int, rng: np.random.Generator):
    ...
    if axiom_type == 0:
        ...
        # CR2
        c1, c2, d = rng.integers(
            0, n_concepts, 3
        )  # don't use TOP and BOT here, it makes no sense
...
def extract_dataset(
    reasoner: ElNaive, max_shortest_proof_length: int
...
def split_dataset(
    ...
    *,
    ...
    for idx, reasoner in enumerate(reasoners):
        pos_train, neg_train, pos_test, neg_test = extract_dataset(
            reasoner, complexity_threshold
        )
    ...
    print(
        "Validation", len(validation), "#pos", sum(label for _, _, label in validation)
    )
