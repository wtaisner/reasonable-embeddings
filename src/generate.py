    ...
    def random_expr(*, max_depth):
        def gen(d):
            ...
            if op is None or d >= max_depth:
                return (
                    rng.choice([TOP, BOT])
                    if rng.random() < p_top_bot
                    else int(rng.integers(n_concepts))
                )
            if op == NOT:
                return op, gen(d + 1)
            if op in {AND, OR}:
...
def add_queries(
    ...
    *,
    ...
    t, i = tick(), 0
    while i < n_queries:
        if timeout(t, i, min_speed):
            break
        rprint(
            f"{fmtprog(i+1, n_queries)} extra queries (total balance {answers_avg.get():.2f})"
        )
        axiom = generate()
        if (
            axiom in queries_set
            or (answer := check_axiom_safe(reasoner, axiom)) is None
        ):
            continue

        if force_balance and (
            answers_avg.get() < 0.5
            and answer is False
            or answers_avg.get() > 0.5
            and answer is True
        ):
            continue

        queries_set.add(axiom)
        queries.append(axiom)
        answers_avg.append(answer)
...
def add_axioms(
    reasoner, onto, *, n_axioms, generate, max_unsat=0.1, min_speed=DEFAULT_MIN_SPEED
):
    Nc = onto.n_concepts
...
def gen_onto(
    *,
    ...
    generate = AxiomGenerator(
        rng=rng, n_concepts=Nc, n_roles=Nr, max_depth=3, p_atomic=p_atomic
    )
    reasoner = Reasoner(n_concepts=Nc, n_roles=Nr, timeout=REASONER_TIMEOUT)
    onto = Onto(tbox=set(), n_concepts=Nc, n_roles=Nr)
    add_axioms(
        reasoner, onto, n_axioms=n_axioms, generate=generate, min_speed=min_speed
    )
    ...
    if len(X) < 3 * len(onto.tbox):
        return None  # too few queries :(
    return onto, X, y
...
def generate_and_split_dataset(
    *, train_ontos, test_ontos, valid_size=0.2, save=None, **params
):
    train_onto, train_queries, train_answers = generate_dataset(
        n_ontos=train_ontos, **params
    )
    test_onto, test_queries, test_answers = generate_dataset(
        n_ontos=test_ontos, **params
    )
...
def serialize_ontos(ontos):
    return [
        dict(
            n_concepts=onto.n_concepts,
            n_roles=onto.n_roles,
            tbox=[to_prefix(x) for x in onto.tbox],
        )
        for onto in ontos
    ]
...
def deserialize_ontos(ontos):
    return [
        Onto(
            n_concepts=onto["n_concepts"],
            n_roles=onto["n_roles"],
            tbox={parse_prefix(x) for x in onto["tbox"]},
        )
        for onto in ontos
    ]
...
def deserialize_dataset(data):
    return (
        deserialize_ontos(data["train_ontos"]),
